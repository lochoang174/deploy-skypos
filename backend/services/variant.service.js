...
module.exports.deleteVariant = async (id) => {
    ...
    const checkVariantInTransaction = await ProductTransaction.findOne({ Variant: id });
    if (checkVariantInTransaction) {
        throw new CustomError("Can not delete variant that is in transaction!", 400);
    }
