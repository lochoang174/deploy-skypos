...
exports.createVariant = async (req, res, next) => {
    ...
    fs.mkdirSync(path.resolve("backend/public/images/" + variant.Product + "/" + variant._id), {
        recursive: true,
    });
    images.forEach((image) => {
        fs.renameSync(
            path.resolve("backend/public/images/" + variant.Product + "/" + image),
            path.resolve("backend/public/images/" + variant.Product + "/" + variant._id + "/" + image)
        );
    });
...
exports.updateVariant = async (req, res, next) => {
    ...
    if (newImages.length > 0) {
        ...
        newImages.forEach((image) => {
            fs.renameSync(
                path.resolve("backend/public/images/" + image),
                path.resolve("backend/public/images/" + variant.Product + "/" + variant._id + "/" + image)
            );
        });
    }
