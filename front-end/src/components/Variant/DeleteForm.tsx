...
export default function DeleteForm(props: TProps) {
    ...
    const { totalVariants, pagination, selectedFilters, error } = useAppSelector((state) => state.variant);
    const handleSubmit = async () => {
        await axios
            .delete(`/variant/${props.currentVariant?._id}`)
            .then(() => {
            ...
            .catch((err) => {
                message.error(error || "Can not delete variant that is in transaction!");
            });
    };
    return (
        <>
